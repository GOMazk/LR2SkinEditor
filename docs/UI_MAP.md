...
`pending-changes`는 SEDrawPendingChanges가 소유하는 앱 전체 저장/종료 검토창이며,
`object-workflow`는 Inspector의 WORKSPACE::drawObjectWorkflowActions다.
기본 전문 탭의 노출과 두 제작 시작점은 [Workflow UI](WORKFLOW_UI.md)를 따른다.
