...
int CSVltoa(CSVbuf* oBuf, int radix) {
	for (int i = 0; i < 30; i++) {
		ltoa(oBuf->val[i], oBuf->str[i], radix);
	}
	return 1;
}
...
void MD5byte(char **iStr, uint len, char *oByte){
	...
	*iStr = (char *)piVar12;
	undefined* const byteBuffer = reinterpret_cast<undefined*>(piVar12);
	memset(byteBuffer + len, 0, iVar21);
	piVar16 = reinterpret_cast<int*>(byteBuffer + iVar21 + len);
	*(byteBuffer + len) = 0x80;
	*piVar16 = len * 8;
	piVar16[1] = len >> 0x1d;
	iVar20 = 0x67452301;
